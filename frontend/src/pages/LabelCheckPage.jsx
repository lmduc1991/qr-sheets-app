...
const MULTI_LABEL_CHECK_MSG = "There are multirow in the label check log";
...
export default function LabelCheckPage() {
  ...
  // Existing check uses these as First Label / Second Label.
  // New "Check many for one" uses the same settings as Column A / Column B.
  ...
  const [mode, setMode] = useState("oneForOne"); // oneForOne | manyForOne
  ...
  // Many-for-one state: scan Column B once, then check many Column A labels.
  const [fixedColumnBValue, setFixedColumnBValue] = useState("");
  const [lastColumnAValue, setLastColumnAValue] = useState("");
  ...
  const loadColumns = async () => {
    ...
    try {
      ...
      setSetupMsg("Columns loaded. Choose Column A and Column B.");
    } catch (e) {
  ...
  const savePageSetup = () => {
    ...
    if (!String(firstLabelColumn || "").trim()) return setSetupErr("Column A / First Label column is required.");
    if (!String(secondLabelColumn || "").trim()) return setSetupErr("Column B / Second Label column is required.");
  ...
  const applyOneForOneResult = async (res) => {
    if (!res?.found) {
    ...
    try {
      await startScanner("label-check-first-reader", async (value) => {
        ...
        await startScanner("label-check-second-reader", async (value2) => {
          ...
          await applyOneForOneResult(res);
        });
      });
    } catch (e) {
      await resetFlow();
  ...
  const beginManyForOne = async () => {
    ...
    try {
      await startScanner("label-check-many-b-reader", async (valueB) => {
        setFixedColumnBValue(valueB);
        setStatus("Column B scanned. Now scan a Column A label.");
        setError("");
        clearResultOnly();
        setStep("scanManyA");

        // Wait for the Column A scanner container to render before mounting html5-qrcode.
        await sleep(150);
        await scanNextColumnA(valueB);
      });
    } catch (e) {
      await resetFlow();
      setError(e?.message || "Failed to start scanner.");
  ...
  const scanNextColumnA = async (overrideColumnBValue = "") => {
    const valueB = String(overrideColumnBValue || fixedColumnBValue || "").trim();
    if (!valueB) {
      await beginManyForOne();
      return;
    }

    setStatus("Scan Column A label.");
    setError("");
    setLastColumnAValue("");
    clearResultOnly();
    setStep("scanManyA");
    ...
    try {
      await startScanner("label-check-many-a-reader", async (valueA) => {
        setLastColumnAValue(valueA);
        setStatus("Checking labels...");
        setError("");

        const res = await getLabelCheckRowsByOneToMany({
          spreadsheetId: settings?.labelCheckSpreadsheetId,
          sheetName: settings?.labelCheckSheetName,
          columnA: settings?.labelCheckFirstLabelColumn,
          columnB: settings?.labelCheckSecondLabelColumn,
          columnAValue: valueA,
          columnBValue: valueB,
        });
        ...
        if (!res?.found) {
          popup("Label Not Match");
          setStatus("Label not match. Column B is still kept; scan another Column A label.");
          setStep("manyResult");
          return;
        }
        ...
        const rows = Array.isArray(res.rows) ? res.rows : [];
        if (rows.length > 1) {
          popup(MULTI_LABEL_CHECK_MSG);
          setStatus("Multiple matching rows found. Column B is still kept; scan another Column A label.");
          setStep("manyResult");
          return;
        }
  ...
  return (
    <div className="page">
      ...
      {(setupOpen || !ready) && (
        <div className="card" style={{ marginTop: 10 }}>
          ...
          <div style={{ marginTop: 10, fontSize: 13, opacity: 0.8 }}>
            Existing check: scan Column A then Column B. Check many for one: scan Column B once, then scan many Column A labels.
          </div>
        </div>
      )}
      ...
      {step === "idle" && (
        <div className="card">
          <h3>Choose check function</h3>
          <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
            <button className={mode === "oneForOne" ? "primary" : ""} onClick={beginOneForOne} disabled={!ready}>
              Start normal label check
            </button>
            <button className={mode === "manyForOne" ? "primary" : ""} onClick={beginManyForOne} disabled={!ready}>
              Check many for one
            </button>
          </div>
          {!ready && (
      ...
      {step === "scanFirst" && (
        <div className="card">
          <p>
            Scan First Label QR / Column A (<strong>{settings?.labelCheckFirstLabelColumn}</strong>)
          </p>
      ...
      {step === "scanSecond" && (
        <div className="card">
          <div style={{ marginBottom: 10 }}>
            <strong>First label / Column A:</strong> {firstScanned}
          </div>
          <p>
            Scan Second Label QR / Column B (<strong>{settings?.labelCheckSecondLabelColumn}</strong>)
          </p>
      ...
      {step === "scanManyB" && (
        <div className="card">
          <p>
            Scan Column B QR one time (<strong>{settings?.labelCheckSecondLabelColumn}</strong>)
          </p>
          <div id="label-check-many-b-reader" />
          <button style={{ marginTop: 10 }} onClick={resetFlow}>
            Cancel
          </button>
        </div>
      ...
      {step === "scanManyA" && (
        <div className="card">
          <div style={{ marginBottom: 10 }}>
            <strong>Fixed Column B:</strong> {fixedColumnBValue}
          </div>
          <p>
            Scan Column A QR to check (<strong>{settings?.labelCheckFirstLabelColumn}</strong>)
          </p>
          <div id="label-check-many-a-reader" />
          <button style={{ marginTop: 10 }} onClick={resetFlow}>
            Cancel
          </button>
        </div>
      ...
      {step === "selectSize" && (
        <div className="card">
          ...
          <div style={{ marginBottom: 12 }}>
            <strong>Second label / Column B:</strong> {secondScanned}
          </div>
      ...
      {step === "result" && matchedRecord && (
        <div className="card">
          ...
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 12 }}>
            <button className="primary" onClick={beginOneForOne}>
              Scan another
            </button>
      ...
      {step === "manyResult" && (
        <div className="card">
          <div style={{ display: "grid", gap: 8, marginBottom: 12 }}>
            ...
            <div>
              <strong>Last Column A:</strong> {lastColumnAValue || ""}
            </div>
            {matchedRowIndex ? (
              <div>
                <strong>Matched row:</strong> {matchedRowIndex}
          ...
          {matchedRecord ? (
            ...
            <div className="alert alert-error">No matched record for the last Column A scan.</div>
          )}

          <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 12 }}>
            <button className="primary" onClick={() => scanNextColumnA()}>
              Scan next Column A
            </button>
            <button onClick={beginManyForOne}>Change Column B</button>
            <button onClick={resetFlow}>Done</button>
          </div>
        </div>
      )}
