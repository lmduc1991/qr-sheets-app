import { Html5Qrcode, Html5QrcodeSupportedFormats } from "html5-qrcode";

function getPreferredQrbox() {
  const vw = typeof window !== "undefined" ? window.innerWidth || 360 : 360;
  const vh = typeof window !== "undefined" ? window.innerHeight || 640 : 640;
  const side = Math.floor(Math.min(vw, vh) * 0.72);
  const clamped = Math.max(180, Math.min(320, side));
  return { width: clamped, height: clamped };
}

function pickBackCamera(cameras = []) {
  if (!Array.isArray(cameras) || cameras.length === 0) return null;

  const ranked = [...cameras].sort((a, b) => {
    const la = String(a?.label || "").toLowerCase();
    const lb = String(b?.label || "").toLowerCase();

    const score = (label) => {
      if (label.includes("back")) return 5;
      if (label.includes("rear")) return 4;
      if (label.includes("environment")) return 3;
      if (label.includes("wide")) return 2;
      return 0;
    };

    return score(lb) - score(la);
  });

  return ranked[0] || cameras[0] || null;
}

async function tryStart(html5QrCode, cameraConfig, config, onSuccess, onError) {
  await html5QrCode.start(cameraConfig, config, onSuccess, onError);
  return true;
}

export async function startQrScanner({ elementId, onScan, fps = 10 }) {
  const el = document.getElementById(elementId);
  if (!el) throw new Error("Scanner container not ready.");

  el.innerHTML = "";

  const qr = new Html5Qrcode(elementId, {
    formatsToSupport: [Html5QrcodeSupportedFormats.QR_CODE],
    verbose: false,
  });

  let stopped = false;
  let lastValue = "";
  let lastTs = 0;

  const config = {
    fps,
    qrbox: getPreferredQrbox(),
    aspectRatio: 1,
    rememberLastUsedCamera: true,
    disableFlip: false,
    experimentalFeatures: {
      useBarCodeDetectorIfSupported: false,
    },
  };

  const onSuccess = async (decodedText) => {
    const value = String(decodedText || "").trim();
    if (!value) return;

    const now = Date.now();
    if (value === lastValue && now - lastTs < 1200) return;
    lastValue = value;
    lastTs = now;

    await onScan(value);
  };

  const onError = () => {};

  const cameraAttempts = [
    { facingMode: { exact: "environment" } },
    { facingMode: "environment" },
    { facingMode: { ideal: "environment" } },
  ];

  let started = false;
  let lastErr = null;

  for (const cameraConfig of cameraAttempts) {
    try {
      await tryStart(qr, cameraConfig, config, onSuccess, onError);
      started = true;
      break;
    } catch (e) {
      lastErr = e;
    }
  }

  if (!started) {
    try {
      const cameras = await Html5Qrcode.getCameras();
      const picked = pickBackCamera(cameras);
      if (picked?.id) {
        await tryStart(qr, picked.id, config, onSuccess, onError);
        started = true;
      }
    } catch (e) {
      lastErr = e;
    }
  }

  if (!started) {
    try {
      const cameras = await Html5Qrcode.getCameras();
      const fallback = cameras?.[0];
      if (fallback?.id) {
        await tryStart(qr, fallback.id, config, onSuccess, onError);
        started = true;
      }
    } catch (e) {
      lastErr = e;
    }
  }

  if (!started) {
    try {
      await qr.clear();
    } catch {
      // ignore
    }
    throw new Error(lastErr?.message || "Failed to start scanner.");
  }

  return {
    stop: async () => {
      if (stopped) return;
      stopped = true;

      try {
        await qr.stop();
      } catch {
        // ignore
      }

      try {
        await qr.clear();
      } catch {
        // ignore
      }
    },
  };
}
